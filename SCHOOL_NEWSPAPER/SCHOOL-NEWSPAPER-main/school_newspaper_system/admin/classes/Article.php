...
class Article
{
    ...
    {
        $stmt = $this->db->prepare('INSERT INTO articles (title,body,image,author_id) VALUES (?,?,?,?)');
        return $stmt->execute([$title, $body, $image, $author_id]);
    }
    public function all()
    {
        $stmt = $this->db->query('SELECT a.*, u.name AS author_name FROM articles a JOIN users u ON a.author_id=u.id ORDER BY a.created_at DESC');
